...
from ..services.leaderboard import get_family_leaderboard, get_child_all_time_stats
...
async def get_leaderboard(
    ...
    leaderboard = await get_family_leaderboard(db, current_user.family_id)

    # Calculate family stats
    total_assigned = sum(e.get("tasks_total", 0) for e in leaderboard if "tasks_total" in dir(locals()))
    total_completed = sum(e.get("tasks_completed", 0) for e in leaderboard if "tasks_completed" in dir(locals()))
